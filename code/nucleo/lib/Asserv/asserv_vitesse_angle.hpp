...
void lecture_Distance_Angle(const float Vitesse,const float W,const float Te,float *Distance, float *Angle); //lecture de la distance parcourue, et de l'angle parcourue
...
float Asserv_Position(const float Position, const float ConsPosition,int reset); //Asserv de Position Distance 
