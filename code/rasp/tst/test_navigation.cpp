...
void test_navigation()
{
    ...
    std::vector<obstacle> list_obstacles=World::fillVector();
    ...
    std::vector<Node> test_path=Navigation::Astar(test_navigation, Navigation(Manche_1),list_obstacles);
    ...
    std::vector<Node> test_path_obstacle=Navigation::Astar(test_navigation, Navigation({45,51}),list_obstacles);
    assert(test_path_obstacle.size()==0);
    print_success();

    printf("Test si nous sommes à la destination ... \n");
    std::vector<Node> test_path_destination=Navigation::Astar(test_navigation, test_navigation,list_obstacles);
    assert(test_path_destination.size()==0);
    print_success();

    printf("Test de déplacement plus complexe ... \n");
    test_path=Navigation::Astar(test_navigation, Navigation(Manche_1),list_obstacles);
    assert(test_path[0].x==Phare.x && test_path[0].y==Phare.y);
    test_path=Navigation::Astar(Navigation(Manche_1), Navigation(Manche_2),list_obstacles);
    test_path=Navigation::Astar(Navigation(Manche_2), Navigation(Phare),list_obstacles);
    test_path=Navigation::Astar(Navigation(Phare), Navigation(Port),list_obstacles);
    assert(test_path[test_path.size()-1].x==Port.x && test_path[test_path.size()-1].y==Port.y);
    good_port(test_path[test_path.size()-1].x, test_path[test_path.size()-1].y, Port.x, Port.y);
    print_success();

    printf("Test sur l'affichage d'un chemin ... \n");
    Navigation::Print_path(test_path);
    print_success();


    printf("Test sur la transmission du chemin à l'asservissement ... \n");
    int back=-5;
    back=Navigation::Navigate_to_asserv(test_path, Navigation(Manche_1), list_obstacles);
    assert(back!=-5);
    print_success();

    printf("Test sur la fonction back_effect \n");
    printf("Si back=0, normal si il ne se passe rien ...");
    Navigation::back_effect(0, Navigation(Manche_1), list_obstacles);
    print_success();
    printf("Si back=1, demande la position du robot et renvoie l'information ... \n");
    Navigation::back_effect(1, Navigation(Manche_1), list_obstacles);
    print_success();
